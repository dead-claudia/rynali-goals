...
const rewardReplacements = new Map([
    ...
    ['Girl month 1 ("end of stream" added 2022-05-25)',
        trust(html`Girl month 1<br>("end of stream" added 2022-05-25)`)],
])
...
function statusRange(d) {
    ...
    if (min === "") {
        if (d.reward === "") return ""
        // If the goal wasn't met, no use in showing anything here.
        if (d.date_achieved_min === "") return ""
        if (d.date_achieved_min === "failed") return ""
